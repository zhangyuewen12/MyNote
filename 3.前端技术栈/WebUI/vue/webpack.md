...
module.exports = {
  ...
  <!-- 这个属性指定了 HTML 模板文件的路径。在构建过程中，Webpack 会使用该模板来生成最终的 HTML 文件。-->
  template: path.resolve(__dirname,'../template/module.index.html')
  <!--这个属性指定了生成的 HTML 文件的名称和相对路径。在构建完成后，生成的 HTML 文件会存储在指定的相对路径中，这个路径相对于输出目录。->
  filename: 'testmodule/testmodule.html'
};
```
