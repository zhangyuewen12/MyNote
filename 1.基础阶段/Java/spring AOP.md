...
<img src="/Users/zyw/Library/Application Support/typora-user-images/image-20230725103223691.png" alt="image-20230725103223691" style="zoom:50%;" />

OOP面向对象编程，针对业务处理过程的实体及其属性和行为进行抽象封装，以获得更加清晰高效的逻辑单元划分。而AOP则是针对业务处理过程中的切面进行提取，它所面对的是处理过程的某个步骤或阶段，以获得逻辑过程的中各部分之间低耦合的隔离效果。这两种设计思想在目标上有着本质的差异。

<img src="/Users/zyw/Library/Application Support/typora-user-images/image-20230725103234992.png" alt="image-20230725103234992" style="zoom:50%;" />
...
![image-20230725103346636](/Users/zyw/Library/Application Support/typora-user-images/image-20230725103346636.png)
...
<img src="/Users/zyw/Library/Application Support/typora-user-images/image-20230725103544703.png" alt="image-20230725103544703" style="zoom:50%;" />
...
<img src="/Users/zyw/Library/Application Support/typora-user-images/image-20230725103712731.png" alt="image-20230725103712731" style="zoom:50%;" />
...
### [#](#切入点-pointcut-的申明规则) 切入点（pointcut）的申明规则?
...
# 基于注释的方式，写一个Spring AOP的例子。

在Java中，可以使用基于注解的方式实现Spring AOP。下面是一个简单的示例，演示如何在Spring中使用基于注解的AOP来实现日志记录的功能。在这个例子中，我们将在一个简单的服务方法上添加日志记录的切面。

假设我们有一个简单的服务接口`UserService`和其实现类`UserServiceImpl`：
...
现在，我们希望在`addUser`方法执行时添加日志记录。我们将使用Spring AOP通过注解的方式来实现。

首先，创建一个自定义注解`@Loggable`，用于标记需要添加日志记录的方法：
...
接下来，创建一个切面类`LoggingAspect`，使用注解来定义通知逻辑：
...
在上述切面类中，我们定义了一个前置通知`logBefore`，它使用`@Before`注解和`@annotation(Loggable)`表达式来匹配所有使用`@Loggable`注解标记的方法，并在这些方法执行前输出日志。

最后，在Spring配置中启用Aspect自动代理，以便切面生效：

applicationContext.xml (假设使用XML配置方式):
...
现在，当我们在`UserServiceImpl`的方法上添加`@Loggable`注解时，该方法的执行就会被日志记录切面捕获并输出日志：
...
在上述示例中，`addUser`方法使用了`@Loggable`注解，因此在调用`addUser`方法时，会触发切面中的前置通知，输出日志信息。这样，我们通过基于注解的方式成功实现了简单的日志记录功能的AOP。